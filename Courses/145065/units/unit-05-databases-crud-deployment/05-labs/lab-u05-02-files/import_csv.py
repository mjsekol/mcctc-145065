...
When DATABASE_URL is set, the import goes to that PostgreSQL database
instead and --db is ignored. [VERIFY on a lab machine with PostgreSQL]
...
def main(argv=None):
    ...
    if not database_url and not pathlib.Path(args.db).exists():
        print(f"No database at {args.db}. Run: python manage.py init --db {args.db}")
        return 1
    conn = db.connect({"DATABASE_URL": database_url, "DATABASE_PATH": args.db})
    try:
