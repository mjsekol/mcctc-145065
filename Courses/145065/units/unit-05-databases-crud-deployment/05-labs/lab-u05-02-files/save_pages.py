...
This script creates, edits, and deletes records. Run the production-style
server (serve_local.py) against a throwaway database, never the one you care about:

    $env:APP_ENV = "production"
    $env:SECRET_KEY = python -c "import secrets; print(secrets.token_hex(32))"
    $env:DATABASE_PATH = "$env:TEMP\line3_pages.db"
    python manage.py init --db $env:DATABASE_PATH
    python serve_local.py --port 8681

Then, in another terminal:

    python save_pages.py --port 8681

To save the 500 page, start serve_local.py against a database file that has
no tables, and run:

    python save_pages.py --port 8681 --only-500
...
def main():
    ...
    parser.add_argument("--only-500", action="store_true",
                        help="save the error page from a server whose database has no tables")
    ...
    if args.only_500:
        save("error-500.html", *browser.request("/"))
        return 0
    ...
    # Larger than MAX_CONTENT_LENGTH (64 KB), so the server refuses it with 413.
    save("error-413.html", *browser.post("/issues/new", dict(NEW_ISSUE, description="x" * 70000)))
    status, _, body = browser.request("/health")
    print(f"{status} /health -> {body.decode('utf-8').strip()} (JSON, not saved)")
