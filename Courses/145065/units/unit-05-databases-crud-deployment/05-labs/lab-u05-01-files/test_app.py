"""
test_app.py · stage w09_database · run from this folder:

    python -m unittest -v

Every test builds a fresh database in a temporary folder from schema.sql and
seed.sql. No test touches instance/line3.db. Standard library plus Flask's
test client.
"""
...
from validation import (DOWNTIME_MAX, NOTE_MINUTES_MAX, NOTE_MINUTES_MIN,  # noqa: E402
...
class TempDatabase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        old_path = app.config["DATABASE_PATH"]
        self.addCleanup(app.config.__setitem__, "DATABASE_PATH", old_path)
        app.config.update(TESTING=True, DATABASE_PATH=str(self.db_path))
...
class SchemaTests(TempDatabase):
    ...
    def test_only_db_py_touches_sqlite(self):
        ...
        for path in HERE.glob("*.py"):
            if path.name in ("db.py", "test_app.py"):
                continue
...
class ReadTests(TempDatabase):
    def test_pages_answer(self):
        for url in ["/", "/issues", "/issues/new", "/issues/3", "/issues/3/edit",
                    "/issues/3/delete", "/equipment", "/equipment/OV-01", "/reports", "/trace"]:
